---
title: "Proposal for an IIF workshop on Open Source for Forecasting"
format: html
---

In the field of forecasting, the use of open-source software such as R, Python, and Julia has become increasingly dominant throughout the entire forecasting workflow. These tools have played a pivotal role in democratizing access to advanced forecasting capabilities. To capitalize on the potential of these open-source solutions, we are delighted to propose a dynamic and enriching IIF sponsored workshop  on open-source for forecasting to be held on June 24-28, 2023.
...
tibble::tribble(
  ...
  "Venue", "TBC(University of Montpelier, France or Cardiff University UK)",
  "Number of expected participants", "40",
  "Travel and accommodation support",	"for 15 participants"
) |> kable(col.names = NULL) |> kable_classic()
```
...
) |> kable(col.names = NULL,caption ="Course fee") |> kable_styling()
```
...
The workshop is aimed at Ph.D. students, researchers and practitioners within forecasting who are already using open-source for basic forecasting and who would like to start using ope-source for forecasting at a more advanced level. In order for potential participants to benefit the most, they should consider themselves eligible if you are comfortable with reading and transforming data, installing and using third-party packages and plotting using standard R graphics and being familiar with principles of forecasting.

We offer places for participants from all over the world.

We can accommodate ~40 participants. Selection criteria include correct entry requirements, motivation to attend the course as well as gender and geographical balance. 


## Budget

The registration fee would be free speakers.

```{r budget, echo=FALSE}
library(kableExtra)
tibble::tribble(
  ~Expense, ~Number,~`Unit Price`,~Total,
  "Catering", 40, 120,4800,
  "Galla Dinner", 40, 60,2400,
  ...
  "Total", NULL, NULL,24200,
) |> kable() |> kable_classic()
```

We expect to have `$5000` income from the workshop fee, therefore	we are requesting a grant of `$19,200` from the IIF towards the costs of the running this workshop.

## Potential list of participants

1. Rob J. Hyndman, 2. Ivan Svetunkov, 3. Bahman Rostami-Tabar 3. Mitchell O'Hara-Wild (ECR), 4. Tobiac Liboschik, 5. Daniele Girolimetto (ECR), 6. Dario Azzimonti, 7. Nickalus Redell, 8. Ronit Jaiswal. 9. Angelo Canty, 10. Jeffrey A. Ryan, 11. Paul-Christian Bürkner, 12. Mingzhi Shi (ECR), 13. Harsha Charma (ECR), 14. Resul Akay, 15. 
